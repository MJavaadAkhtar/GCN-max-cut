...
def run_gnn_training(dataset, net, embed, optimizer, number_epochs, tol, patience, prob_threshold):
    """
    ...
    best_bitstring = torch.zeros((dataset[0][0].number_of_nodes(),)).type(dataset[0][1].dtype).to(dataset[0][1].device)
    best_loss = loss_func(best_bitstring.float(), dataset[0][1])
    ...
    for key, value in dataset.items():
        ...
        for epoch in range(number_epochs):
            ...
            probs = net(value[0], inputs)[:, 0]  # collapse extra dimension output from model

            # build cost value with QUBO cost function
            loss = loss_func(probs, value[1])
            ...
            if count >= patience:
                if epoch > 100000:
                    break
                print(f'{key}: Stopping early on epoch {epoch} (patience: {patience})')
                count = 0
                break
